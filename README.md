...
1. `Tuto` by `Bonifarz`. This map is highly recommended in the first place to learn basic and advanced tricks.
...
reservoir - if you open the map in the edit mode some sounds are missing; the map does not fix your record when reaching the end<br>
Reaction - the map cannot be played. You are getting stuck in the spectator mode. (The map is playable and can be finished in RE_1.4)<br>
polarization - missing skybox and some textures, the ceiling is clipping at the end of the map so that allows to change a route and break the map due to the wrong sequence. This is a singleplayer map can be well played in Race.<br>
...
ladderrace - no checkpoints<br>
holidays - texture error; console error unknown command setpixelparam. https://forum.freegamedev.net/viewtopic.php?f=73&t=3393<br>
<br>
escape_sp_final<br>
escape_sp_v1<br>
escapeeasyfinal<br>
...
sinai - there is a shortcut that cuts the map in a half (you can easily jump to a finish window from the below platform)<br>
Reaction - the map is very interesting but it has some weird custom physics. It was meant to be so: https://forum.freegamedev.net/viewtopic.php?f=73&t=3886. It'd be nice to have a version of this map with the standard physics.<br>
...
carve - you can get stuck at the end of the map near that vertical water pool<br>
...
### Here is the original description by TristamK
